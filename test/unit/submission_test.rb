...
class SubmissionTest < ActiveSupport::TestCase
  ...
  test "user.submission.vote(:type, submission_object) should add a vote with the passed type" do
    user = FactoryGirl.create(:valid_user, :id => 1535353153)
    submission = FactoryGirl.create(:valid_submission)
  ...
  test "user.submission.vote(:type, submission_object) should return false if a user double votes" do
    user = FactoryGirl.create(:valid_user, :id => 5530873153)
    submission = FactoryGirl.create(:valid_submission)
    vote = user.vote(:up, submission)
  ...
  test "self.total_votes should return fresh vote count" do
    ...
    submission = FactoryGirl.create(:valid_submission, :id => 7401482)
    counter = 0
    (10..40).each do |i|
      user = FactoryGirl.create(:valid_user, :email => "totalvoter-#{i}@metajury.com", :username => "totalvoter#{i}")
      counter = counter + 1
  ...
  test "self.up_votes should return a fresh 'up' vote count" do
    ...
    counter = 0
    (10..40).each do |i|
      user = FactoryGirl.create(:valid_user, :email => "upvoter-#{i}@metajury.com", :username => "upvoter#{i}")
      counter = counter + 1
      user.vote(:up, submission)
    end
  ...
  test "self.down_votes should return a fresh 'down' vote count" do
    ...
    counter = 0
    (10..40).each do |i|
      user = FactoryGirl.create(:valid_user, :email => "downvoter-#{i}@metajury.com", :username => "downvoter#{i}")
      counter = counter + 1
  ...
  test "user.friendly_url should return a url without the path" do
    submission = FactoryGirl.create(:valid_submission, :url => 'http://www.cnn.com/example?i=13531')
    assert(submission.friendly_url === 'http://www.cnn.com', "Output was: #{submission.friendly_url}")
  end

  test "submissions should be upvoted by default during creation" do
    user = FactoryGirl.create(:valid_user, :email => 'initial_upvote@metajury.com', :username => 'initial_upvote')
    submission = FactoryGirl.create(:valid_submission, :user_id => user.id)
