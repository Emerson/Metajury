...
class UserTest < ActiveSupport::TestCase
  ...
  test "self.refresh_auth_tokens should save new auth tokens for every user" do
    ...
    token_array = []
    (1..50).each do |n|
      user = FactoryGirl.create(:valid_user, :email => "auth#{n}@token.com", :username => "auth#{n}")
  ...
  test "new users should have a login_count of 0" do
    user = FactoryGirl.create(:user, :confirmed => true, :token => nil, :email => 'new_user_count@text.com')
    assert_equal(0, user.login_count)
  end

  test "user.login_count should increment every login" do
    FactoryGirl.create(:valid_user, :email => 'emerson.lackey@gmail.com')
  ...
  test "admin? should be true when user_level equals 'admin'" do
    FactoryGirl.create(:admin, :email => 'admin@application.com')
  ...
  test "validate user by token" do
    token = FactoryGirl.create(:valid_user).token
  ...
  test "confirmed user are allowed to login" do
    FactoryGirl.create(:valid_user, :email => 'emerson.lackey@gmail.com', :password => 'password')
    user = User.login('emerson.lackey@gmail.com', 'password')
  ...
  test 'non-admin users cannot change other users accounts' do
    user = FactoryGirl.create(:valid_user, :email => 'naughty-user@bad.com', :username => 'baduserchange')
